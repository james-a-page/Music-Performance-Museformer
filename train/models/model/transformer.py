...
class Transformer(nn.Module):
	...
	def make_museformer_mask(self, q, k, q_bar_idx, k_bar_idx, struct_related_bars=[1,2,4,8,16,32], use_eos_as_summary=True, EOS_idx = 3):
		...
		# Make a triangle mask first then remove extra unwanted bits
		mask = torch.tril(torch.ones(len_q, len_k)).type(torch.BoolTensor).to(self.device)
		bar_count = 0
		for i, mask in enumerate(mask):
			if q[i] == q_bar_idx or (use_eos_as_summary and q[i] == EOS_idx):
				...
				if bar_count > 0:
					...
					for struct_bar in struct_related_bars:
						if bar_count > struct_bar:
							temp_mask[q_bar_pointers[bar_count -
													 struct_bar]][1] = 1
						else:
							break
					mask = torch.logical_and(mask, temp_mask)
