...
class REMI(MIDITokenizer):
    ...
    def midi_to_tokens(self, midi: MidiFile, *args, **kwargs) -> List[Union[int, List[int]]]:
        ...
        # Changes below:
        # Going to make changse under the assumption that all our tracks will be piano
        # - therefore can merge all tracks into a single track of tokens to ensure consistent format for the model
        if len(tokens) <= 1:
            return tokens[0]
        # Step over all token tracks incrementatlly.
        #   Within each bar:
        #       Iterate until position token reached in each track
        #       Choose lowest position value - add tokens from that stream to the list until reach another postion token
        #       Repeat until reach a bar
        #   Repeat for each bar
        merged_tokens = []
        cur_tokens = []
        for track in tokens:
            cur_tokens.append((0,track[0]))
        #TODO event currently just the int token, need to convert to Event obj for this to work
        while not all(event == None for (_, event) in cur_tokens):
            #Check if all are bar tokens, if so we add a bar token and move on.
            if all((event == None or self.vocab.token_type(event) == "Bar") for (_, event) in cur_tokens):
                for (_, event) in cur_tokens:
                    if event != None:
                        merged_tokens.append(event)
                        break

                cur_tokens = [(j+1, tokens[i][j+1]) if event != None else (j, event) for i, (j, event) in enumerate(cur_tokens)]
            #In all other cases we want to add the lowest position token and then the following values until another bar token is reached.
            else:
                last_pos = -1
                #Find lowest positon values:
                min_pos = np.Infinity
                track_pointers = []
                # print(cur_tokens)
                for i, (j, track_token) in enumerate(cur_tokens):
                    # track_token = self.vocab.token_type(track_token)
                    if track_token != None:
                        if self.vocab.token_type(track_token) == "Rest":
                            # merged_tokens.append(track_token)
                            cur_tokens[i] = (j+1, tokens[i][j+1])
                            track_pointers = []
                            break

                        if self.vocab.token_type(track_token) == "Position":
                            #If we have 2 equivalent min position we want to add both the tokens at that position marker.
                            token_value = int(self.vocab.token_to_event[
                                track_token].split("_")[1])
                            # print(token_value)
                            if token_value == min_pos:
                                track_pointers = [i]
                            elif token_value < min_pos:
                                track_pointers = [i]
                                min_pos = token_value
                #Add a position pointer
                if track_pointers:
                    if min_pos != last_pos:
                        merged_tokens.append(cur_tokens[track_pointers[0]][1])
                        last_pos = min_pos
                    #For each set of tokens at that position pointer add all tokens until we reach a bar token.
                for i in track_pointers:
                    (j, _) = cur_tokens[i]
                    j += 1
                    while tokens[i][j] != None and self.vocab.token_type(tokens[i][j]) != "Bar":
                        merged_tokens.append(tokens[i][j])
                        j += 1
                        #If j overshoots the end of the list then we append a None value to that token list to indicate end of that track
                        if j >= len(tokens[i]):
                            tokens[i].append(None)
                    cur_tokens[i] = (j, tokens[i][j])
    ...
    def track_to_tokens(self, track: Instrument) -> List[int]:
        ...
        #Changed to allow for rest-starts
        previous_note_end = 0
